  ...
  try {
    ...
    if (metricsMode === 'custom') {
      ...
      if (configError || !config || config.length === 0) {
        return NextResponse.json({ error: 'Metric configuration not found' }, { status: 404 })
      }
    }
    ...
    if (jobsToCreate.length > 0) {
      const { error: jobsError } = await supabase.from('analysis_jobs').insert(jobsToCreate)

      if (jobsError) {
        console.error('Error creating jobs:', jobsError)
        // Clean up the run
        await supabase.from('program_runs').delete().eq('id', runId)
        return NextResponse.json({ error: 'Failed to create analysis jobs' }, { status: 500 })
      }
    }

...
export class JobRunner {
  ...
  constructor(supabase: SupabaseClient, appSecretKey: string, workerId?: string) {
  ...
  private async processNextJob(runId?: string): Promise<void> {
    try {
      ...
      if (program.credential_id) {
        ...
        // Decrypt cookie
        try {
          const decryptedCookie = decryptFromStorage(credential.cookie_encrypted, this.appSecretKey)
      ...
      try {
        ...
        if (!adapter) {
          await this.jobQueue.updateJobStatus(
            job.id,
            'failed',
            `Unknown source type: ${program.source_type}`,
          )
          return
        }
        ...
        if (errorMessage.includes('401') || errorMessage.includes('403')) {
          await this.jobQueue.updateJobStatus(
            job.id,
            'failed',
            'Authentication failed - please update credentials',
          )
        } else {
